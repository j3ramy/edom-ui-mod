...
public class AtmScreen extends ContainerScreen<AtmContainer> {
    ...
    private final ResourceLocation GUI = new ResourceLocation(EdomMod.MOD_ID, "textures/gui/atm_gui.png");
    ...
    private String pin = pinPlaceholder;
    private String info = "Geben Sie Ihren PIN ein";
    ...
    public AtmScreen(AtmContainer screenContainer, PlayerInventory inv, ITextComponent title) {
        ...
        String[] data = AtmHttp.getAccountData(OWNER);
        if(data.length == 0){
            ...
            ACCESS_DENIED = Integer.parseInt(data[4]);
        }
    }
    ...
    void drawWithdrawMenu(MatrixStack matrixStack){
        ...
        this.addButton(new Button(159, 45, 25, 20, new StringTextComponent("5"), (onclick) -> withdraw(5)));

        this.addButton(new Button(186, 45, 25, 20, new StringTextComponent("10"), (onclick) -> withdraw(10)));

        this.addButton(new Button(213, 45, 25, 20, new StringTextComponent("20"), (onclick) -> withdraw(20)));

        this.addButton(new Button(240, 45, 25, 20, new StringTextComponent("50"), (onclick) -> withdraw(50)));

        this.addButton(new Button(159, 67, 25, 20, new StringTextComponent("100"), (onclick) -> withdraw(100)));

        this.addButton(new Button(186, 67, 25, 20, new StringTextComponent("200"), (onclick) -> withdraw(200)));

        this.addButton(new Button(213, 67, 25, 20, new StringTextComponent("500"), (onclick) -> withdraw(500)));
    ...
    void drawDepositMenu(MatrixStack matrixStack){
        ...
        this.addButton(new Button(240, 72, 80, 20, new StringTextComponent("Einzahlen"), (onclick) -> deposit()));
    }
    ...
    private void withdraw(int value){
        if(value <= BALANCE){
            if(!areSlotOccupied()){
                if(AtmHttp.sendWithdrawData(te.getPos(), value)){
                    AtmHttp.withdraw(OWNER, value, te.getPos());

                    if(te.getWorld() != null)
                        te.getWorld().playSound(player , te.getPos(), ModSoundEvents.ATM_WITHDRAW.get(), SoundCategory.BLOCKS, 1, 1);

                    closeScreen();
                }
                else{
                    infoColor = infoColorError;
                    info = "Auszahlung fehlgeschlagen";
                }
            }
            else{
                infoColor = infoColorError;
                info = "Leeren Sie die Ausgabeslots";
            }

        }
        else{
            infoColor = infoColorError;
            info = "Kontodeckung unzureichend";
        }
    }
    ...
    void deposit(){
        if(depositValue == 0){
            infoColor = infoColorError;
            info = "Bitte legen Sie Noten ein";
            return;
        }

        AtmHttp.deposit(OWNER, depositValue, te.getPos());

        if(te.getWorld() != null)
            te.getWorld().playSound(player , te.getPos(), ModSoundEvents.ATM_DEPOSIT.get(), SoundCategory.BLOCKS, 1, 1);
